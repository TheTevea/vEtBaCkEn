...
class DistrictsController extends AppController {
    ...
    function add() {
        ...
        if (!empty($this->data)) {
            for ($i = 0; $i < sizeof($_POST['name']); $i++) {
                ...
                $this->Helper->saveUserActivity($user['User']['id'], 'District', 'Save Add New', $this->District->id);
            }
    ...
    function delete($id = null) {
        ...
        mysql_query("UPDATE `districts` SET `is_active`=2, `modified`='".date("Y-m-d H:i:s")."', `modified_by`=".$user['User']['id']." WHERE `id`=".$id.";");
