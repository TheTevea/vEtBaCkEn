...
class ChartAccountGroupsController extends AppController {
    ...
    function delete($id = null) {
        ...
        if(!$notAllowDelete){
            mysql_query("UPDATE `chart_account_groups` SET `is_active`=2, `modified`='".date("Y-m-d H:i:s")."', `modified_by`=".$user['User']['id']." WHERE `id`=".$id.";");
    ...
    function status($id = null, $status = 0) {
        ...
        $this->ChartAccountGroup->updateAll(
                array('ChartAccountGroup.is_active' => $status),
                array('ChartAccountGroup.id' => $id)
        );
        echo MESSAGE_DATA_HAS_BEEN_SAVED;
        exit;
    }
