...
class ChartAccountsController extends AppController {
    ...
    function delete($id = null) {
        ...
        if(!$notAllowDelete){
            mysql_query("UPDATE `chart_accounts` SET `is_active`=2, `modified`='".date("Y-m-d H:i:s")."', `modified_by`=".$user['User']['id']." WHERE `id`=".$id.";");
    ...
    function status($id = null, $status = 0) {
        ...
        $this->ChartAccount->updateAll(
                array('ChartAccount.is_active' => $status),
                array('ChartAccount.id' => $id)
        );
        echo MESSAGE_DATA_HAS_BEEN_SAVED;
        exit;
    }
