...
class BranchCurrenciesController extends AppController {
    ...
    function add() {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->BranchCurrency->save($this->data)) {
                ...
                // Convert to REST
                $branchCode   = $this->Helper->getSQLSyncCode("branches", $this->data['BranchCurrency']['branch_id']);
        ...
        $currencyCenters = ClassRegistry::init('CurrencyCenter')->find('list',
                        array(
                            'conditions' => array('CurrencyCenter.is_active = 1')
                        )
        );
        // User Activity
    ...
    function delete($id = null) {
        ...
        $dateNow  = date("Y-m-d H:i:s");
        $user = $this->getCurrentUser();
        $branchCurrency = $this->BranchCurrency->read(null, $id);
        ...
        $restCode[$r]['dbtodo']  = 'branch_currencies';
        $restCode[$r]['actodo']  = 'ut';
        $restCode[$r]['con']     = "sys_code = '".$branchCurrency['BranchCurrency']['sys_code']."'";
        // Save File Send
        $this->Helper->sendFileToSync($restCode);
        // User Activity
