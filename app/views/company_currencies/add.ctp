...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#CompanyCurrencyAddForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#CompanyCurrencyAddForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('Company Currency', 'Add', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Company Currency', 'Add', 1, '');
                    // alert message
            ...
            <td>
                <div class="inputContainer">
                    <select name="data[CompanyCurrency][company_id]" id="CompanyCurrencyCompanyId" class="validate[required]">
                        ...
                        ?>
                        <option value="<?php echo $company['Company']['id']; ?>" dcur="<?php echo $company['Company']['currency_center_id']; ?>"><?php echo $company['Company']['name']; ?></option>
                        <?php } ?>
                    </select>
                </div>
            </td>
