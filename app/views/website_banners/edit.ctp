...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#WebsiteBannerEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#WebsiteBannerEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>'){
                    createSysAct('WebsiteBanner', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('WebsiteBanner', 'Edit', 1, '');
                    // alert message
    ...
    <table style="width: 100%">
        <tr>
            ...
            <td valign="top">
                ...
                <input type="file" id="WebsiteBannerPhoto" name="photo" />
            </td>
        </tr>
        <tr>
            <td colspan="2" style="text-align: left;">
                ...
                ?>
                <img id="WebsiteBannerPhotoDisplay" src="<?php echo $photo; ?>" style=" max-width: 140px; max-height: 140px;" />
            </td>
        </tr>
    </table>
</form>
...
<input type="hidden" id="<?php echo $photoNameHidden; ?>" name="data[WebsiteBanner][photo]" value="<?php echo $this->data['WebsiteBanner']['photo']; ?>" />
<fieldset>
    <legend><?php __(MENU_WEBSITE_BANNER_EDIT); ?></legend>
    <table cellpadding="5">
        <tr>
            ...
            <td>
                <div class="inputContainer">
                    <select name="data[WebsiteBanner][website_type]" id="WebsiteBannerWebsiteType" class="validate[required]">
                        ...
                        <option value="1" <?php if($this->data['WebsiteBanner']['website_type'] == 1){ ?>selected=""<?php } ?>><?php echo "Vireak Buntham"; ?></option>
                        <option value="2" <?php if($this->data['WebsiteBanner']['website_type'] == 2){ ?>selected=""<?php } ?>><?php echo "Buva Sea"; ?></option>
                    </select>
                </div>
            </td>
        </tr>
