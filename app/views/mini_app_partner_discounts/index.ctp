...
<script type="text/javascript">
    var oTableMiniAppPartnerDiscount;
    $(document).ready(function(){
        preventKeyEnter();
        $("#<?php echo $tblName; ?> td:first-child").addClass('first');
        oTableMiniAppPartnerDiscount = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnEditMiniAppPartnerDiscount").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var leftPanel=$(this).parent().parent().parent().parent().parent().parent().parent();
                    var rightPanel=leftPanel.parent().find(".rightPanel");
                ...
                $(".btnChangeStatusMiniAppPartnerDiscount").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    ...
                    $("#dialog").dialog({
                        ...
                        buttons: {
                            ...
                            '<?php echo TABLE_INACTIVE; ?>': function() {
                                $.ajax({
                                    type: "GET",
                                    url: "<?php echo $this->base.'/'.$this->params['controller']; ?>/status/" + id + "/0",
                                    data: "",
                                    beforeSend: function(){
                                        $("#dialog").dialog("close");
                                        $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
                ...
                $(".btnDeleteMiniAppPartnerDiscount").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var name = $(this).attr('name');
                                ...
                                $.ajax({
                                    ...
                                    success: function(result){
                                        ...
                                        if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                                            createSysAct('MiniAppPartnerDiscount', 'Delete', 2, result);
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                                        }else {
                                            createSysAct('MiniAppPartnerDiscount', 'Delete', 1, '');
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span>'+result+'</p>');
                                        }
                                        $("#dialog").dialog({
                                            title: '<?php echo DIALOG_INFORMATION; ?>',
        ...
        $(".btnAddMiniAppPartnerDiscount").click(function(event){
            event.preventDefault();
...
<div class="leftPanel">
    ...
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <div class="buttons">
            <a href="" class="positive btnAddMiniAppPartnerDiscount">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_ADD_MINI_APP_DISCOUNT; ?>
            </a>
        </div>
        ...
        <table id="<?php echo $tblName; ?>" class="table" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th><?php echo MENU_MINI_APP_PARTNER_NAME; ?></th>
                    <th><?php echo TABLE_FIXED_DISCOUNT; ?></th>
                    <th><?php echo TABLE_PERCENT; ?></th>
                    <th><?php echo TABLE_START_DATE; ?></th>
                    <th><?php echo TABLE_END_DATE; ?></th>
