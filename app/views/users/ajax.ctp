 ...
 */
$aColumns = array('id', 'username', 'first_name', 'last_name', 'telephone', 'type');
...
if ($_GET['sSearch'] != "") {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if($aColumns[$i] == 'username' || $aColumns[$i] == 'first_name' || $aColumns[$i] == 'last_name'){
...
/* Customize condition */
$condition = "is_active = 1 AND offline_project_id = 1 AND type = 2";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $row[] =
            ...
            ($allowEdit && $aRow[5] == 2 ? '<a href="" class="btnEditUser" rel="' . $aRow[0] . '" name="' . $aRow[2] . ' ' . $aRow[3] . '"><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
            ...
            ($allowDelete && $aRow[5] == 2 && $aRow[0] > 1 ? '<a href="" class="btnDeleteUser" rel="' . $aRow[0] . '" name="' . $aRow[2] . ' ' . $aRow[3] . '"><img alt="Delete" onmouseover="Tip(\'' . ACTION_DELETE . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    $output['aaData'][] = $row;
}
