...
$link .= str_replace("/t_transportation_types/edit/".$this->data['TTransportationType']['id'],"",$_SERVER['REQUEST_URI']);
?>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#TTransportationTypeEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#TTransportationTypeEditForm").ajaxForm({
        ...
        generateDefaultTableTTransportationType();
        resetDragTTransportationType();
        setDragPositionTTransportationType();
    ...
    function setDragPositionTTransportationType(){
        var TTransportationTypeLayouts = $.parseJSON('<?php echo $this->data['TTransportationType']['layout']; ?>');
        $.each(TTransportationTypeLayouts, function(key, value) {
            var columns = value.col;
            var rowIndex = key + 1;
            var totalColSpan = 0;
            var colIndex = 0;
            var firstCol = replaceNum(columns[0].attr.colspan);
            $.each(columns, function(index, col) { // Columns
                var colspan  = replaceNum(col.attr.colspan);
                if(colspan > 0){
                    for (var rmCol = 1; rmCol < colspan; rmCol++) {
                        var removeCol = colIndex;
                        $("#layoutDropTTransportationType").find("tr").eq((rowIndex - 1)).find("td").eq(removeCol).remove();
                    }
                    $("#layoutDropTTransportationType").find("tr").eq((rowIndex - 1)).find("td").eq(index).attr('colspan', colspan);
                    if(index == 0){
                        colIndex = 2;
                    } else {
                        if(firstCol == 2){
                ...
                if(col.value != ''){
                    ...
                    if(isNaN(replaceNum(col.value)) == true){
                        drag = '#labelDropTTransportationType'+col.value;
                    } else {
                        drag = '#chairTTransportationType'+col.value;
                    }
                    if(col.label != undefined){
                        label = col.label;
                    }
                    var colIndexSpan = colIndex;
                    var drop = '#layoutDropTTransportationType'+rowIndex+'BT'+colIndexSpan;
                    autoDragTTransportationType(drag, drop, label);
                }
            });
        });
    }
    
    function autoDragTTransportationType(drag, drop, label){
        ...
        $(drag).find('.removeChairTTransportationType').show();
        $(drag).find('.layoutDropSeat').text(dropId);
        $(drag).find('.layoutDropSeat').hide();
        $(drag).css('visibility', 'hidden');
        // Clone Drag
        var cloneDrag   = $(drag).html();
        ...
        eventRemoveDragPosition();
        // Set Boat Number & Label
        $(drop).find("span.BoatNumber").text(number);
        $(drop).find("span.BoatLabel").text(label);
        $(drop).find("span.BoatNumber").hide();
        $(drop).find("span.BoatLabel").hide();
    ...
    <table style="width: 100%;">
        <tr>
            ...
            <td rowspan="3" style="vertical-align: top;">
                <div class="inputContainer">
                    <?php echo $this->Form->input('amenity_id', array('selected' => $amenitySellected, 'name' => 'data[amenity_id]', 'style' => 'width: 400px;', 'multiple' => true, 'div' => false, 'label' => false)); ?>
                </div>
            </td>
        </tr>
        <tr>
    ...
    <div style="width: 850px; float: left; border: 1px solid #000; min-height: 400px; padding: 5px; margin-left: 10px;">
        <table cellpadding="5" cellspacing="0" style="width: 100%;">
            <tr>
                ...
                <td style="width: 70px;"><input type="text" id="layoutDropTTransportationTypeRow" name="data[TTransportationType][total_row]" value="<?php echo $this->data['TTransportationType']['total_row']; ?>" style="width: 90%;" class="integer" /></td>
                <td style="width: 80px;">Total Column:</td>
                <td style="width: 70px;"><input type="text" id="layoutDropTTransportationTypeColumn" name="data[TTransportationType][total_column]" value="<?php echo $this->data['TTransportationType']['total_column']; ?>" style="width: 90%;" class="integer" /></td>
                <td>
                    <div class="buttons">
