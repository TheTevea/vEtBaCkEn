...
<fieldset>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <?php 
                $sqlJourney = mysql_query("SELECT id FROM t_journeys WHERE company_id = ".$this->data['Company']['id']." AND status > 0 LIMIT 1");
                $sqlTicket  = mysql_query("SELECT id FROM t_tickets WHERE company_id = ".$this->data['Company']['id']." AND status > 0 LIMIT 1");
                if(!mysql_num_rows($sqlJourney) && !mysql_num_rows($sqlTicket)){
        ...
        <tr>
                ...
                <select id="userCompany" multiple="multiple" style="width: 300px; height: 200px;">
                    ...
                    if($user['User']['type'] == 2){
                        if(!empty($user['User']['offline_project_id'])){
                            $userCon = " AND users.offline_project_id = ".$user['User']['offline_project_id'];
                        } else {
                            $userCon = " AND users.offline_project_id = 0";
                        }
                    } else {
                        $userCon = "";
                    }
                    $querySource = mysql_query("SELECT id,CONCAT(first_name,' ',last_name) AS full_name FROM users WHERE is_active=1 AND id NOT IN (SELECT user_id FROM user_companies WHERE company_id=".$this->data['Company']['id'].")".$userCon);
            ...
            <td style="vertical-align: top;">
                <select id="userCompanySelected" name="data[Company][user_id][]" multiple="multiple" style="width: 300px; height: 200px;">
                    <?php
                    $queryDestination=mysql_query("SELECT DISTINCT user_id, (SELECT CONCAT(first_name,' ',last_name) FROM users WHERE id = user_companies.user_id".$userCon.") AS full_name FROM user_companies WHERE company_id = ".$this->data['Company']['id']);
