...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        oTableCompany = $("#<?php echo $tblName; ?>").dataTable({
			...
			buttons: {
                ...
                $(".btnUpdateStatusCompany").click(function(event){
                    event.preventDefault();
                    var id   = $(this).attr('rel');
                    var name = $(this).attr('name');
                    $("#dialog").html('<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_CONFIRM_DELETE; ?> <b>' + name + '</b>?</p>');
			...
			buttons: {
                            '<?php echo ACTION_ACTIVE; ?>': function() {
                                updateStatusCompany(id, '2');
                            },
                            '<?php echo ACTION_INACTIVE; ?>': function() {
                                updateStatusCompany(id, '3');
                            },
                            '<?php echo ACTION_CANCEL; ?>': function() {
                                $(this).dialog("close");
                            }
    ...
    function updateStatusCompany(id, status){
        $.ajax({
            type: "GET",
            ...
            success: function(result){
                ...
                // alert message
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                    createSysAct('Company', 'Update Status', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Company', 'Update Status', 1, '');
                    // alert message
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span>'+result+'</p>');
                }
                $("#dialog").dialog({
