...
<script type="text/javascript">
    $(document).ready(function(){
        preventKeyEnter();
        $("#MiniAppPartnerEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#MiniAppPartnerEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('MiniAppPartner', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('MiniAppPartner', 'Edit', 1, '');
            ...
            <td>
                <div class="inputContainer">
                    <select name="data[MiniAppPartner][status]" id="MiniAppPartnerStatus" class="validate[required]" style="width: 260px; height: 25px;">
                        <option value="1" <?php echo $this->data['MiniAppPartner']['status'] == 1 ? 'selected="selected"' : ''; ?>><?php echo TABLE_ACTIVE; ?></option>
                        <option value="0" <?php echo $this->data['MiniAppPartner']['status'] == 0 ? 'selected="selected"' : ''; ?>><?php echo TABLE_INACTIVE; ?></option>
                    </select>
                </div>
            </td>
        </tr>
