...
<script type="text/javascript">
    var oTableMiniAppPartner;
    $(document).ready(function(){
        preventKeyEnter();
        $("#<?php echo $tblName; ?> td:first-child").addClass('first');
        oTableMiniAppPartner = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnViewMiniAppPartner").click(function(event){
                ...
                $(".btnEditMiniAppPartner").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var leftPanel=$(this).parent().parent().parent().parent().parent().parent().parent();
                    var rightPanel=leftPanel.parent().find(".rightPanel");
                ...
                $(".btnChangeStatusMiniAppPartner").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                ...
                $(".btnDeleteMiniAppPartner").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var name = $(this).attr('name');
                                ...
                                $.ajax({
                                    ...
                                    success: function(result){
                                        ...
                                        if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                                            createSysAct('MiniAppPartner', 'Delete', 2, result);
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                                        }else {
                                            createSysAct('MiniAppPartner', 'Delete', 1, '');
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span>'+result+'</p>');
                                        }
                                        $("#dialog").dialog({
                                            title: '<?php echo DIALOG_INFORMATION; ?>',
        ...
        $(".btnAddMiniAppPartner").click(function(event){
            event.preventDefault();
...
<div class="leftPanel">
    ...
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <div class="buttons">
            <a href="" class="positive btnAddMiniAppPartner">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_ADD_MINI_APP; ?>
            </a>
        </div>
