...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#TJourneyPriceDefaultEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#TJourneyPriceDefaultEditForm").ajaxForm({
...
//                    createSysAct('TJourneyPriceDefault', 'Edit', 2, result);
//                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
//                }else {
                    createSysAct('TJourneyPriceDefault', 'Edit', 1, '');
                    // alert message
...
<fieldset>
    ...
    <table>
        <tr>
            <td><label for="TJourneyPriceDefaultApplyTo"><?php echo TABLE_APPLY_TO; ?> <span class="red">*</span> :</label></td>
            <td>
                <div class="inputContainer">
                    <select name="data[TJourneyPriceDefault][apply_to]" id="TJourneyPriceDefaultApplyTo" style="height: 25px;" class="validate[required]">
                        <option value=""><?php echo INPUT_SELECT; ?></option>
                        <option value="1" <?php if($this->data['TJourneyPriceDefault']['apply_to'] == 1){ ?>selected=""<?php } ?>><?php echo TABLE_ALL; ?></option>
                        <option value="2" <?php if($this->data['TJourneyPriceDefault']['apply_to'] == 2){ ?>selected=""<?php } ?>><?php echo MENU_MAIN_BRANCH; ?></option>
                    </select>
                </div>
            </td>
        </tr>
        <tr id="divTJourneyPriceDefaultMainBranchId" <?php if($this->data['TJourneyPriceDefault']['apply_to'] == 1){ ?>style="display: none;"<?php } ?>>
