    ...
    <tbody>
        <?php
            ...
            foreach ($this->data as $exchangeRate) {
                $sqlComCurrency = mysql_query("SELECT name, symbol FROM currency_centers WHERE id = ".$exchangeRate['companies']['currency_center_id']);
                $rowComCurrency = mysql_fetch_array($sqlComCurrency);
                ?>
                    <tr>
                        <td class="first"><?php echo ++$index; ?></td>
                        <td><?php echo $exchangeRate['companies']['name']; ?></td>
                        <td><?php echo $rowComCurrency['name']." (".$rowComCurrency['symbol'].")"; ?></td>
                        <td>1.00</td>
                        <td><?php echo $exchangeRate['currency_centers']['name']." (".$exchangeRate['currency_centers']['symbol'].")"; ?></td>
