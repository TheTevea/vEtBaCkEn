...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#<?php echo $mainBranch; ?>, #<?php echo $destinationFrom; ?>, #<?php echo $destinationTo; ?>, #<?php echo $province; ?>, #<?php echo $destGroup; ?>, #<?php echo $agency; ?>").chosen({width: 220});
        ...
        $("#<?php echo $bookingType; ?>").unbind("change").change(function(){
            var bookingType = $(this).val();
            var link = "";
            if(bookingType == ""){
                link = "getAgencyByType";
            } else {
                if(bookingType != 1 && bookingType != 2 && bookingType != 6 && bookingType != 7 && bookingType != 8){
                    var type    = 1; // Online
                    var payment = 1; // Prepaid
                    if(bookingType == 9 || bookingType == 10 || bookingType == 11){ // Postpaid
                        payment = 2;
                    }
                    if(bookingType == 3 || bookingType == 9){ // Api
                        type = 3;
                    }
                    if(bookingType == 5 || bookingType == 11){ // Offline
                        type = 2;
                    }
                    link = "getAgencyByType/"+type+"/"+payment;
                }
            }
            if(link != ""){
                $(".salesSummaryFilterAgency").show();
                $.ajax({
                    type: "POST",
                    url: "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/"+link,
                    data: '',
                    ...
                    success: function(result){
                        $(".loader").attr("src","<?php echo $this->webroot; ?>img/layout/spinner-placeholder.gif");
                        $("#<?php echo $agency; ?>").html(result);
                        $("#<?php echo $agency; ?>").trigger("chosen:updated");
                    }
                });
            } else {
                $("#<?php echo $agency; ?>").html('<option value=""><?php echo TABLE_ALL; ?></option>');
                $("#<?php echo $agency; ?>").trigger("chosen:updated");
                $(".salesSummaryFilterAgency").hide();
            }
        });
    });
...
<div class="legend">
        ...
        <table style="width: 100%;">
            <tr>
                ...
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <?php
                        echo $this->Form->select($createdBy, $users, null, array('escape' => false, 'name' => 'created_by', 'empty' => INPUT_SELECT, 'class' => $chosen)); 
                        ?>
                    </div>
                </td>
                ...
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <select id="<?php echo $viewby; ?>">
                            ...
                            <option value="2" selected="">Summary</option>
                        </select>
                    </div>
                </td>
                <td></td>
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                <td style="width: 8%;"><label for="<?php echo $bookingType; ?>"><?php echo "Booking Type"; ?>:</label></td>
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <select id="<?php echo $bookingType; ?>" name="booking_type">
                            <option value="">All</option>
                            <option value="1">Walk In</option>
                            <option value="2">Website</option>
                            <option value="6">Mini App</option>
                            <option value="7">App</option>
                            <option value="8">Terminal</option>
                            <option value="3">Agent APi (Prepaid)</option>
                            <option value="4">Agent Online (Prepaid)</option>
                            <option value="5">Agent Offline (Prepaid)</option>
                            <option value="9">Agent APi (Postpaid)</option>
                            <option value="10">Agent Online(Postpaid)</option>
                            <option value="11">Agent Offline(Postpaid)</option>
                        </select>
                    </div>
                </td>
                ...
                <td style="width: 7%;" class="salesSummaryFilterAgency"><label for="<?php echo $agency; ?>"><?php echo MENU_AGENT; ?>:</label></td>
                <td style="width: 15%;" class="salesSummaryFilterAgency">
                    <div class="inputContainer">
                        <?php
                        $empty = TABLE_ALL;
