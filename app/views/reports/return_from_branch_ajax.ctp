...
/* DB table to use */
$sTable = "t_tickets INNER JOIN branches ON branches.id = t_tickets.branch_id INNER JOIN currency_centers ON currency_centers.id = t_tickets.currency_center_id";
...
/* Customize condition */
$condition = 't_tickets.journey_type = 2 AND t_tickets.status = 2';
if ($data[1] != '') {
...
$condition != '' ? $condition .= ' AND ' : '';
if ($data[4] != '') {
    $condition .= "t_tickets.branch_id = '" . $data[4]. "'";
} else {
