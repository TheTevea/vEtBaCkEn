...
<script type="text/javascript">
    $(document).ready(function(){
        $("#<?php echo $bookingType; ?>").chosen({width: 350});
        <?php
        if($user['User']['type'] != 2){
        ?>
        ...
        $("#<?php echo $btnSearch; ?>").click(function(){
            ...
            if(isFormValidated){
                ...
                if((issueDateFrom != "" && issueDateTo != "") || (travelDateFrom != "" && travelDateTo != "")){
                    ...
                    $("#salesTicketOnlineCompany").val($("#<?php echo $bookingType; ?>").val());
                    $.ajax({
                        type: "POST",
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                ...
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <input type="hidden" name="book_type" id="salesTicketOnlineCompany" />
                        <?php 
                        $comEmpty = false;
                        if(count($companies) != 1){
                            $comEmpty = TABLE_ALL;
                        }
                        echo $this->Form->select($bookingType, $companies, null, array('escape' => false, 'data-placeholder' => $comEmpty, 'width' => '200', 'multiple' => true, 'name' => '')); 
                        ?>
                ...
                if($user['User']['type'] != 2){
                ?>
                <td><input type="hidden" name="main_branch" value="" /></td>
                <?php
                } else {
                ?>
