...
$condition = "offline_project_id = 1 AND t_tickets.t_agent_id != 55";
if($_POST['status']!='') {
...
if($_POST['book_type']!='') {
    $condition .= " AND company_id IN (".$_POST['book_type'].")";
    $_POST['book_type'] = str_replace(",", "-", $_POST['book_type']);
}
...
<div id="<?php echo $printArea; ?>">
    ...
    $totalMarkup = 0;
    $totalBonus  = 0;
    $sqlTicket = mysql_query("SELECT id, (IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount, total_vat, t_agent_id, t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission, IFNULL(t_tickets.total_bonus, 0) AS total_bonus, t_tickets.total_seat, t_tickets.date, t_tickets.is_round_trip FROM t_tickets WHERE ".$condition);
    while($rowTicket = mysql_fetch_array($sqlTicket)){
        ...
        if($rowAgency['commission_type'] == 1){ // Set Percentage
            $totalCommission += replaceThousand(number_format((($rowTicket['total_amount'] + $rowTicket['total_vat']) * $rowAgency['commission']) / 100, 3));
            ...
            if($rowTicket['is_round_trip'] == 1){
                $agePrice  = $rowJou['agent_round_price'] * $rowTicket['total_seat'];
                if($rowSeat['nationally'] == 2){
                    $agePrice  = $rowJou['agent_round_price_foreigner'] * $rowTicket['total_seat'];
                }
            } else {
            ...
            if(mysql_num_rows($sqlPA)){
                ...
                if($rowPAPrice['price_type'] == 1){
                    if($rowTicket['is_round_trip'] == 1){
                        if($rowSeat['nationally'] == 2){    
                            $agePrice  = $rowPAPrice['round_agency_price_foreigner'] * $rowTicket['total_seat'];
                        } else {
                            $agePrice  = $rowPAPrice['round_agency_price'] * $rowTicket['total_seat'];
                        }
                    } else {
                        if($rowSeat['nationally'] == 2){    
                    ...
                    if($rowTicket['is_round_trip'] == 1){
                        if($rowSeat['nationally'] == 2){  
                            $agePrice  = $agePrice + ($rowPAPrice['round_agency_price_foreigner'] * $rowTicket['total_seat']);
                        } else {
                            $agePrice  = $agePrice + ($rowPAPrice['round_agency_price'] * $rowTicket['total_seat']);   
                        }
                    } else {
                        if($rowSeat['nationally'] == 2){  
            ...
            $totalCommission += replaceThousand(number_format($commission, 3));
        } else { // Fixed Amount
            $totalCommission += replaceThousand(number_format($rowAgency['commission'], 3));
        }
    ...
    $sqlTicket3M = mysql_query("SELECT id, (IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount, total_vat, t_agent_id, t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission, IFNULL(t_tickets.total_bonus, 0) AS total_bonus, t_tickets.total_seat, t_tickets.date, t_tickets.is_round_trip FROM t_ticket_3months AS t_tickets WHERE ".$condition);
    while($rowTicket = mysql_fetch_array($sqlTicket3M)){
        $sqlAgency = mysql_query("SELECT * FROM t_agents WHERE id = ".$rowTicket['t_agent_id']);
        $rowAgency = mysql_fetch_array($sqlAgency);
        $sqlSeat   = mysql_query("SELECT nationally FROM t_ticket_detail_3months WHERE t_ticket_id = ".$rowTicket['id']." LIMIT 1");
        $rowSeat   = mysql_fetch_array($sqlSeat);
        if($rowAgency['commission_type'] == 1){ // Set Percentage
            $totalCommission += replaceThousand(number_format((($rowTicket['total_amount'] + $rowTicket['total_vat']) * $rowAgency['commission']) / 100, 3));
        } else if($rowAgency['commission_type'] == 2) { // Agency Price
    ...
    $rowSeat = mysql_fetch_array($sqlSeat);
    $sqlSeat3M = mysql_query("SELECT COUNT(t_ticket_details.id) FROM t_ticket_detail_3months AS t_ticket_details INNER JOIN t_ticket_3months AS t_tickets ON t_tickets.id = t_ticket_details.t_ticket_id AND t_tickets.t_agent_id != 55 WHERE ".$condition." AND t_ticket_details.is_active = 1");
    ...
    $totalFare = replaceThousand(number_format($totalAmount + $totalVat, 2));
    $totalCommission = replaceThousand(number_format($totalCommission, 2));
    $totalBonus = replaceThousand(number_format($totalBonus, 2));
    ?>
    <table cellpadding="5" cellspacing="0" style="width: 100%;">
        <tr>
            ...
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format($rowSeat[0] + $rowSeat3M[0], 0); ?></td>
            <td colspan="<?php echo $colspan; ?>"></td>
        </tr>
        <tr>
            <td style="font-size: 14px; width: 80px;"><?php echo TABLE_TOTAL_FARE; ?>:</td>
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format($totalFare, 2); ?> $</td>
            <td style="font-size: 14px; width: 130px;"><?php echo TABLE_TOTAL_COMMISSION; ?>:</td>
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format($totalCommission, 3); ?> $</td>
            <td style="font-size: 14px; width: 130px;"><?php echo "Total Bonus"; ?>:</td>
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format($totalBonus, 2); ?> $</td>
            <td style="font-size: 14px; width: 100px;"><?php echo TABLE_NET_PAYMENT; ?>:</td>
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format(($totalFare - $totalCommission - $totalBonus), 3); ?> $</td>
            <?php
            if($isApi == 0){
            ?>
        ...
        <table id="<?php echo $tblName; ?>" class="table" style="width: 120%;">
            <thead>
                <tr>
                    ...
                    <th style="width: 75px !important; font-size: 10px; text-align: center; <?php if($isApi == 1){ ?>display: none;<?php } ?>"><?php echo "Bonus"; ?></th>
