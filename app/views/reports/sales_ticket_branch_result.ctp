...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/salesTicketBranchAjax/<?php echo str_replace("/", "|||", implode(';', $_POST)); ?>",
            ...
            "fnDrawCallback": function(oSettings, json) {
                $("#<?php echo $tblName; ?> .colspanParentSalesTicket").parent().attr("colspan", 15);
                ...
                $("#<?php echo $tblName; ?> .colspanParentSalesTicket").parent().next().remove();
                // Total
                $("#<?php echo $tblName; ?> .colspanTotalSalesTicket").parent().attr("colspan", 9);
                ...
                $("#<?php echo $tblName; ?> .colspanTotalSalesTicket").parent().next().remove();
                // Grand Total
                $("#<?php echo $tblName; ?> .colspanTotalGrandSalesTicket").parent().attr("colspan", 9);
...
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . REPORT_SALES_TICKET_BRANCH . '</b><br /><br />';
    if($_POST['status']!='') {
    ...
    $sqlTicket = mysql_query("SELECT COUNT(id) AS total_booked, SUM(IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0) + IFNULL(t_tickets.total_vat, 0)) AS total_amount FROM t_tickets WHERE ".$condition);
