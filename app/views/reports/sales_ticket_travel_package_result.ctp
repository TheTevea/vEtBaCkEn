...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/salesTicketTravelPackageAjax/<?php echo str_replace("/", "|||", implode(';', $_POST)); ?>",
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/sales_ticket_travel_package<?php echo $user['User']['id']; ?>.csv", "_blank");
        });
    });
</script>
<div id="<?php echo $printArea; ?>">
    <?php
    $msg = '<b style="font-size: 18px;">' . REPORT_TRAVEL_PACKAGE_TICKET . '</b><br /><br />';
    ...
    if($_POST['travel_package_id']!='') {
        $condition .= " AND t_tickets.travel_package_order_id = ".$_POST['travel_package_id'];
    }

    if($_POST['customer_name']!='') {
        $condition .= " AND travel_package_orders.name = '".$_POST['customer_name']."'";
    }

    echo $this->element('/print/header-report',array('msg'=>$msg));
    $sqlTicket = mysql_query("SELECT COUNT(t_tickets.id) AS total_booked, SUM(IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount 
                              FROM t_tickets 
        ...
        <table id="<?php echo $tblName; ?>" class="table" style="width: 100%;">
            <thead>  
                <tr>
                    ...
                    <th style="width: 170px !important; font-size: 10px;"><?php echo "Package Type"; ?></th>
                    <th style="width: 170px !important; font-size: 10px;"><?php echo "Package Code"; ?></th>
                    <th style="width: 170px !important; font-size: 10px;"><?php echo "Package Name"; ?></th>
                    <th style="width: 170px !important; font-size: 10px;"><?php echo "Package Tel"; ?></th>
