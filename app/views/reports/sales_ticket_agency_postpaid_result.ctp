...
$msg = '<b style="font-size: 18px;">' . REPORT_SALES_TICKET_AGENCY_OFFLINE_POSTPAID . '</b><br /><br />';
if($_POST['status']!='') {
...
if($_POST['paid_type']!='') {
    $condition .= " AND t_tickets.is_agent_paid = ".$_POST['paid_type'];
}
?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/salesTicketAgencyPostpaidAjax/<?php echo str_replace("/", "|||", implode(',', $_POST)); ?>",
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                ?>
                $(".btnAgentOfflinePostPaidUnPaid").unbind("click").click(function(event){
                ...
                ?>
                $("#chkAgencyOfflinePostpaid").unbind("click").click(function(){
                    if($(this).is(':checked')){
                        $(".chkAgencyOfflinePostpaid").attr("checked", true);
                    } else {
                        $(".chkAgencyOfflinePostpaid").attr("checked", false);
                    }
                });
                return sPre;
            },
            "aoColumnDefs": [{
                ...
                if($allowPaid){
                ?>
                "bSortable": false, "aTargets": [ 0, 1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16, 17 ]
                <?php
                } else {
                ?>
                "bSortable": false, "aTargets": [ 0, 1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16 ]
                <?php
                }
                ?>
            }],
                ...
                $("#dialog").dialog({
                    ...
                    buttons: {
                        '<?php echo ACTION_YES; ?>': function() {
                            ...
                            $("#lblBtnAgencyOfflinePostpaidClaim").text("<?php echo ACTION_LOADING; ?>");
                            $.ajax({
                                type: "POST",
                                dataType: "json",
                                url: "<?php echo $this->base.'/'.$this->params['controller']; ?>/agencyOnlinePostpaidClaim/2",
                                data: post,
                                beforeSend: function(){
                                    $("#dialog").dialog("close");
                                    $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
                                ...
                                success: function(result){
                                    ...
                                    oCache.iCacheLower = -1;
                                    <?php echo $oTable; ?>.fnDraw(false);
                                    createSysAct('Report', 'Agency Offline PostPaid Claim', 1, '');
                                    $(".<?php echo $btnClaim; ?>").attr("disabled", false);
                                    $("#lblBtnAgencyOfflinePostpaidClaim").text("Paid & Print");
                                    // alert message
                                    if(result.error == "0"){
                                        $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?></p>');
                                        $.ajax({
                                            type: "GET",
                                            url: "<?php echo $this->base.'/'.$this->params['controller']; ?>/salesTicketAgencyPostpaidPrint/"+result.id,
                                            beforeSend: function(){
                                                $(".loader").attr('src','<?php echo $this->webroot; ?>img/layout/spinner.gif');
                                            },
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/sales_ticket_agency_offline_postpaid<?php echo $user['User']['id']; ?>.csv", "_blank");
        });
    });

    function agencyOnlinePostpaidConvertPost(){
        var post = "";
        $(".chkAgencyOfflinePostpaid").each(function(){
            if($(this).is(':checked')){
                if(post != ''){
                    post += '&';
                }
...
<div id="<?php echo $printArea; ?>">
    ...
    $sqlTicket = mysql_query("SELECT t_tickets.id, 
                              ...
                              t_tickets.t_agent_id, t_tickets.t_journey_id, t_tickets.is_agent_paid, 
    ...
    <table cellpadding="5" cellspacing="0" style="width: 100%;">
        ...
        <tr>
            ...
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format(($totalAmount - $totalNet), 2); ?> $</td>
        ...
        <table id="<?php echo $tblName; ?>" class="table" style="width: 100%;">
            <thead>
                <tr>
                    ...
                    <th style="width: 120px !important; font-size: 10px;"><?php echo "Reference"; ?></th>
                    ...
                    <th style="width: 120px !important; font-size: 10px; <?php if($allowPaid == false){ echo 'display: none;'; } ?>"><?php echo ACTION_ACTION; ?> <input type="checkbox" id="chkAgencyOfflinePostpaid" /></th>
                </tr>
            </thead>
            <tbody>
                <tr>
...
<div class="buttons">
    <button type="button" id="<?php echo $btnClaim; ?>" class="positive">
        ...
        <span id="lblBtnAgencyOfflinePostpaidClaim"><?php echo 'Paid & Print'; ?></span>
    </button>
</div>
<?php
} 
