 ...
 * export to excel
 */
$filename = "public/report/sales_ticket_agency_offline_postpaid" . $user['User']['id'] . ".csv";
...
$excelContent .= "\n\n".TABLE_NO."\t".MENU_AGENT."\t".TABLE_TICKET_CODE."\tReference\t".TABLE_BOOKING_DATE."\t".TABLE_JOURNEY_DATE."\t".REPORT_FROM."\t".REPORT_TO."\t".TABLE_SEAT."\tSelling Price\tNet Price\t".TABLE_COMMISSION."\t".TABLE_MARKUP."\t".TABLE_CREATED."\tPaid Date\tPaid By\t".TABLE_STATUS."\t".ACTION_ACTION;
...
$aColumns = array(
            ...
            'IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0) + IFNULL(t_tickets.total_vat, 0)',
            'IFNULL(agency_balances.debit, 0)', // 10
            'IFNULL(t_tickets.commission, 0)', 
            'IFNULL(t_tickets.total_markup, 0)', 
            't_tickets.created',
            'agency_postpaid_claim_details.created',
            'IFNULL(users.username, "")', // 15
            't_tickets.status', 
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $row[] = number_format(($aRow[9] - $aRow[10]), 2)." ".$aRow[17];
            $excelContent .= "\t".number_format(($aRow[9] - $aRow[10]), 2)." ".$aRow[17];
        } else if ($aColumns[$i] == 'IFNULL(t_tickets.total_markup, 0)') {
            $row[] = number_format($aRow[$i], 2)." ".$aRow[17];
            $excelContent .= "\t".number_format($aRow[$i], 2)." ".$aRow[17];
        } else if ($aColumns[$i] == 'currency_centers.symbol') {
            if($allowPaid){
                if($aRow[16] == 2 && $aRow[19] == 0){
                    $row[] = '<input type="checkbox" value="'.$aRow[0].'" class="chkAgencyOfflinePostpaid" />';
                } else {
                    $row[] = '<a href="#" class="btnAgentOfflinePostPaidUnPaid" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Unpaid" onmouseover="Tip(\'Unpaid\')" src="' . $this->webroot . 'img/button/stop.png" /></a>';
                }   
            } else {
                $row[] = '';
            }
