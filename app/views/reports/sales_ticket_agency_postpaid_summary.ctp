...
<div class="leftPanel">
    <div id="<?php echo $printArea; ?>">
        <?php
        $msg = '<b style="font-size: 18px;">' . REPORT_SALES_TICKET_AGENCY_OFFLINE_POSTPAID . ' (SUMMARY)</b><br /><br />';
        ...
        if($_POST['paid_type']!='') {
            ...
            $conTicket .= " AND t_tickets.is_agent_paid = ".$_POST['paid_type'];
        }
            ...
            <table class="table_print" cellspacing="0">
                <tbody>
                    <tr>
                        ...
                        <th style="width: 7%; font-size: 12px; font-weight: bold; padding: 5px; background: #c8c8c8; text-align: right;"><?php echo "Max Balance"; ?></th>
                        <th style="width: 7%; font-size: 12px; font-weight: bold; padding: 5px; background: #c8c8c8; text-align: right;"><?php echo "ទឹកលុយជំពាក់"; ?></th>
                        ...
                        <th style="width: 9%; font-size: 12px; font-weight: bold; padding: 5px; background: #c8c8c8; text-align: right;"><?php echo "Total Net Price"; ?></th>
                    ...
                    $totalBalanceSell = 0;
                    $datas = array();
                    $sqlTicket = mysql_query("SELECT t_tickets.id, t_tickets.t_agent_id, t_agents.name AS agent_name, t_agents.type AS agent_type, t_agents.commission AS agent_commission, t_agents.commission_type AS agent_commission_type, t_agent_types.name AS agent_group, t_agents.max_balance,
                    ...
                    if(mysql_num_rows($sqlTicket)){
                        while($rowTicket = mysql_fetch_array($sqlTicket)){
                            ...
                            if(array_key_exists($key, $datas)){
                                ...
                                $datas[$key]['total_net']    += $rowTicket['total_net'];
                                ...
                                $datas[$key]['total_commission'] = $rowTicket['total_amount'] - $rowTicket['total_net'];
                                $datas[$key]['total_markup'] = $rowTicket['total_markup'];
                                $datas[$key]['max_balance']  = $rowTicket['max_balance'];
                            }
                        }
                    }
                    if(!empty($datas)){
                        foreach($datas AS $key => $data){
                            ...
                            $balance    = $data['max_balance'] - ($rowBalance[0] * -1);
                            $totalMaxBalance  += $data['max_balance'];
                            $totalBalance     += $balance;
                            $totalBalanceSell += ($rowBalance[0] * -1);
                    ?>
                    <tr>
                        ...
                        <td style="padding: 5px; font-size: 12px; text-align: right;">
                            <?php 
                            echo "$ ".number_format($data['max_balance'], 2);
                            ?>
                        </td>
                        <td style="padding: 5px; font-size: 12px; text-align: right;">
                            <?php 
                            echo "$ ".number_format($rowBalance[0] * -1, 2);
                            ?>
                        </td>
                        <td style="padding: 5px; font-size: 12px; text-align: right;">
                            <?php 
                        ...
                        <td style="padding: 5px; font-size: 12px; text-align: right;">
                            ...
                            echo "$ ".number_format($data['total_net'], 2); 
                            ?>
                        </td>
                        <td style="padding: 5px; font-size: 12px; text-align: right;">
                            <?php 
                    ...
                    <tr>
                        ...
                        <td style="font-size: 12px; font-weight: bold; text-align: right;">$ <?php echo number_format($totalMaxBalance, 2); ?></td>
                        <td style="font-size: 12px; font-weight: bold; text-align: right;">$ <?php echo number_format($totalBalanceSell, 2); ?></td>
                        ...
                        <td style="font-size: 12px; font-weight: bold; text-align: right;">$ <?php echo number_format($totalNet, 2); ?></td>
                    ...
                    ?>
                    <tr>
                        <td colspan="13" style="text-align: center; padding: 5px;"><?php echo TABLE_NO_RECORD; ?></td>
                    </tr>
                    <?php
                    }
                    ?>
