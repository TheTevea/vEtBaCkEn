...
$msg = '<b style="font-size: 18px;">' . MENU_REPORT_SALES_TICKET_WEBSITE . '</b><br /><br />';
if($_POST['status']!='') {
    $condition = "t_tickets.status = ".$_POST['status']." AND (t_tickets.type = 5 OR t_tickets.type = 11)";
} else {
    $condition = "t_tickets.status >= 0 AND (t_tickets.type = 5 OR t_tickets.type = 11)";
}
...
if($_POST['type']!='') {
    if($_POST['type'] == 1){ // Mobile
        $condition .= " AND t_tickets.t_agent_id IS NULL";
...
if($_POST['payment_method']!='') {
    $condition .= " AND online_orders.payment_method_id IN (".$_POST['payment_method'].")";
}
if($_POST['company']!='') {
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        <?php echo $oTable; ?> = $("#<?php echo $tblName; ?>").dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->base.'/'.$this->params['controller']; ?>/salesTicketWebsiteAjax/<?php echo str_replace("/", "|||", implode(',', str_replace(",", "-", $_POST))); ?>",
        ...
        $("#<?php echo $btnExport; ?>").click(function(){
            window.open("<?php echo $this->webroot; ?>public/report/sales_ticket_app_website<?php echo $user['User']['id']; ?>.csv", "_blank");
        });
    });
</script>
<div id="<?php echo $printArea; ?>">
    ...
    $totalBooked = 0;
    $totalMarkup = 0;
    $sqlTicket = mysql_query("SELECT t_tickets.id, (IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount, t_tickets.t_agent_id, t_tickets.t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission 
                              FROM t_tickets 
        ...
        <table id="<?php echo $tblName; ?>" class="table" style="width: 100%;">
            <thead>
                <tr>
                    ...
                    <th style="width: 130px !important; font-size: 10px;"><?php echo "Email"; ?></th>
