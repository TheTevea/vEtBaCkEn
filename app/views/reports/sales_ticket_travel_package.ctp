...
$travelPackage   = "travelPackage" . $rnd;
$customerName    = "customerName" . $rnd;
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#<?php echo $mainBranch; ?>, #<?php echo $destinationFrom; ?>, #<?php echo $destinationTo; ?>, #<?php echo $travelPackage; ?>, #<?php echo $customerName; ?>").chosen({width: 220});
        ...
        $("#<?php echo $btnSearch; ?>").click(function(){
            ...
            var link = 'salesTicketTravelPackageResult';
            if(isFormValidated){
...
<div class="legend">
    <div class="legend_title">
        <?php echo REPORT_TRAVEL_PACKAGE_TICKET; ?>
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                <td style="width: 8%;"><label for="<?php echo $travelPackage; ?>"><?php echo MENU_TRAVEL_PACKAGE; ?>:</label></td>
                <td style="width: 20%;">
                    <div class="inputContainer">
                        <select id="<?php echo $travelPackage; ?>" style="width: 200px;" name="travel_package_id">
                            <option value=""><?php echo TABLE_ALL; ?></option>
                            <?php
                            foreach($travelPackageOrders AS $travelPackageOrder){
                            ?>
                            <option value="<?php echo $travelPackageOrder['travelPackageOrder']['id']; ?>"><?php echo $travelPackageOrder['travelPackageOrder']['package_code']." - ".$travelPackageOrder['travelPackageOrder']['name']." ".$travelPackageOrder['travelPackageOrder']['telephone']; ?></option>
                            <?php
                            }
                            ?>
                        </select>
                ...
                <td style="width: 8%;"><label for="<?php echo $customerName; ?>"><?php echo "Customer Name"; ?>:</label></td>
                <td style="width: 20%;">
                    <div class="inputContainer">
                        <select id="<?php echo $customerName; ?>" style="width: 200px;" name="customer_name">
                            <option value=""><?php echo TABLE_ALL; ?></option>
                            <?php
                            $sqlCus = mysql_query("SELECT name FROM travel_package_orders WHERE status = 2 GROUP BY name ORDER BY name ASC;");
                            while($rowCus = mysql_fetch_array($sqlCus)){
                            ?>
                            <option value="<?php echo $rowCus['name']; ?>"><?php echo $rowCus['name']; ?></option>
                            <?php
                            }
                            ?>
                        </select>
