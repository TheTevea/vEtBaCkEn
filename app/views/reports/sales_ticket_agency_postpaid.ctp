...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#<?php echo $agency; ?>, #<?php echo $destinationFrom; ?>, #<?php echo $destinationTo; ?>, #<?php echo $agencyGroup; ?>, #<?php echo $mainBranch; ?>").chosen({width: 220});
        ...
        $("#<?php echo $btnSearch; ?>").click(function(){
            ...
            if(isFormValidated){
                ...
                if((issueDateFrom != "" && issueDateTo != "") || (travelDateFrom != "" && travelDateTo != "")){
                    var link   = 'salesTicketAgencyPostpaidResult';
                    var viewBy = $("#<?php echo $viewBy; ?>").val();
                    if(viewBy == 2){
                        link = 'salesTicketAgencyPostpaidSummary';
                    }
                    $("#reportAgencyOfflinePostpaidCompany").val($("#<?php echo $company; ?>").val());
                    $.ajax({
                        type: "POST",
        ...
        getDestAgencyOnlinePostPaid();
    });
    
    function getDestAgencyOnlinePostPaid(){
...
<div class="legend">
    <div class="legend_title">
        <?php echo REPORT_SALES_TICKET_AGENCY_OFFLINE_POSTPAID; ?>
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                ...
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <input type="hidden" name="company" id="reportAgencyOfflinePostpaidCompany" />
                        <?php 
                        $comEmpty = false;
                        if(count($companies) != 1){
                            $comEmpty = TABLE_ALL;
                        }
                        echo $this->Form->select($company, $companies, null, array('escape' => false, 'name' => '', 'data-placeholder' => $comEmpty, 'multiple' => true)); 
                        ?>
                    </div>
                </td>
                ...
                <td style="width: 7%;"><label for="<?php echo $mainBranch; ?>"><?php echo MENU_MAIN_BRANCH; ?>:</label></td>
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <?php
                        echo $this->Form->select($mainBranch, $mainBranches, null, array('escape' => false, 'name' => 'main_branch', 'empty' => TABLE_ALL)); 
                        ?>
                    </div>
                </td>
                <td style="width: 7%;"><label for="<?php echo $agencyGroup; ?>"><?php echo "Agency Group"; ?>:</label></td>
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <?php
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                <td style="width: 8%;"><label for="<?php echo $paidType; ?>"><?php echo "Paid Type"; ?>:</label></td>
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <select id="<?php echo $paidType; ?>" name="paid_type">
                            <option value=""><?php echo TABLE_ALL; ?></option>
                            <option value="0"><?php echo "Unpaid"; ?></option>
