...
<script type="text/javascript">
    $(document).ready(function(){
        $("#<?php echo $destinationFrom; ?>").chosen({width: 350});
        $("#<?php echo $destinationTo; ?>").chosen({width: 250});
        ...
        $("#<?php echo $btnSearch; ?>").click(function(){
            ...
            if(isFormValidated){
                ...
                if((issueDateFrom != "" && issueDateTo != "") || (travelDateFrom != "" && travelDateTo != "")){
                    var link = 'salesTicketBkkBuvaResult';
                    var destFrom = "";
                    if($("#<?php echo $destinationFrom; ?>").val() != null){
                        destFrom = $("#<?php echo $destinationFrom; ?>").val();
                    }
                    $("#<?php echo $destinationFrom; ?>Val").val(destFrom);
                    $.ajax({
                        type: "POST",
...
<div class="legend">
    <div class="legend_title">
        <?php echo REPORT_SALES_TICKET_BRANCH; ?> (Bkk & Buva Sea)
    ...
    <div class="legend_content <?php echo $formFilter; ?>">
        <table style="width: 100%;">
            <tr>
                ...
                <td style="width: 25%;">
                    <div class="inputContainer">
                        <input type="hidden" name="destination_from" id="<?php echo $destinationFrom; ?>Val" />
                        <select id="<?php echo $destinationFrom; ?>" multiple="" style="width: 250px;" data-placeholder="All">
                            <?php
                            $sqlDestF = mysql_query("SELECT * FROM t_destinations WHERE id IN (4, 26, 32)");
                            while($rowDestF = mysql_fetch_array($sqlDestF)){
                            ?>
                            <option value="<?php echo $rowDestF['id']; ?>"><?php echo $rowDestF['name']; ?></option>
                            <?php
                            }
                            ?>
                        </select>
                ...
                <td style="width: 15%;">
                    <div class="inputContainer">
                        <select id="<?php echo $destinationTo; ?>" name="destination_to" style="width: 220px;">
                            <option value=""><?php echo TABLE_ALL; ?></option>
                            <?php
                            $sqlDestT = mysql_query("SELECT * FROM t_destinations WHERE id != 26 AND id != 32 AND is_active = 1");
                            while($rowDestT = mysql_fetch_array($sqlDestT)){
                            ?>
                            <option value="<?php echo $rowDestT['id']; ?>"><?php echo $rowDestT['name']; ?></option>
                            <?php
                            }
                            ?>
                        </select>
