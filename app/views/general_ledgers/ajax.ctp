...
/* Customize condition */
$condition = "gl.is_active = 1 AND is_sys=0 AND gl.created_by=" . $userId . " AND " . (!empty($companies) ? "company_id IN (" . implode(",", array_keys($companies)) . ")" : "company_id IN (SELECT company_id FROM user_companies WHERE user_id = ".$user['User']['id'].")");
