...
<script type="text/javascript">
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#AmenityEditForm").validationEngine('attach', {
            isOverflown: true,
            overflownDIV: ".ui-tabs-panel"
        });
        $("#AmenityEditForm").ajaxForm({
            ...
            success: function(result) {
                ...
                if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_COULD_NOT_BE_SAVED; ?>' && result != '<?php echo MESSAGE_DATA_ALREADY_EXISTS_IN_THE_SYSTEM; ?>'){
                    createSysAct('Amenity', 'Edit', 2, result);
                    $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                }else {
                    createSysAct('Amenity', 'Edit', 1, '');
                    // alert message
    ...
    <table cellpadding="5">
        <tr>
            <td colspan="2">
                ...
                ?>
                <input type="hidden" name="data[Amenity][photo]" id="AmenityPhoto" value="<?php echo $this->data['Amenity']['photo']; ?>" />
                <img src="<?php echo $img; ?>" id="AmenityPhotoDisplay" style="width: 128px; height: 128px;" />
            </td>
        </tr>
        <tr>
            <td colspan="2">
