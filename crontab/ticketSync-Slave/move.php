...
$sqlSer = mysql_query("SELECT * FROM offline_servers WHERE 1 LIMIT 1");
if(mysql_num_rows($sqlSer)){
    $rowSer = mysql_fetch_array($sqlSer);
    $keyEnc = md5($rowSer['key'].$rowSer['s_t']);
    ...
    if(mysql_num_rows($sqlReceive)){
        while($rowReceive = mysql_fetch_array($sqlReceive)){
            // Check Exist Code
            $sqlChk = mysql_query("SELECT id FROM offline_server_dones WHERE syn_code = '".$rowReceive['syn_code']."' LIMIT 1");
            if(!mysql_num_rows($sqlChk)){
                $accessSave = true;
                $contents = $rowReceive['contents'];
                $content  = decode($contents, $keyEnc);
