...
include_once('/var/www/VetTicketNF/app/webroot/includes/PayWayApiCheckout.php');
include_once('/var/www/VetTicketNF/app/webroot/includes/AcledaCheckout.php');
include_once("/var/www/VetTicketNF/app/webroot/includes/NewWingConfig.php");

$sqlOrder = mysql_query("SELECT * FROM online_orders WHERE status IN (1,2) AND input_type = 1 AND offline_project_id = 1 AND date_add(`created`, interval 25 minute) < now();");
while($rowOrder = mysql_fetch_array($sqlOrder)){
    ...
    if($rowOrder['payment_method_id'] == 5 || $rowOrder['payment_method_id'] == 6 || $rowOrder['payment_method_id'] == 7){ // ABA
        ...
        if ($curl_errno > 0) {
            ...
            if(is_array($output) && isset($output['status'], $output['description']) && $output['status'] == 0 && $output['description'] == "approved"){
                $checkPayment = true;
            }
        }
        ...
        if(mysql_num_rows($sqlAcledaStatus)){
            while($rowAcledaStatus = mysql_fetch_array($sqlAcledaStatus)){
                ...
                if ($curl_errno > 0) {
                    ...
                    if(is_array($result) && isset($result['result']['code'], $result['result']['errorDetails'], $result['result']['xTran']['paymentTokenid'], $result['result']['xTran']['invoiceid'], $result['result']['xTran']['purchaseAmount']) && $result['result']['code'] == "0" && $result['result']['errorDetails'] == "SUCCESS" && $result['result']['xTran']['paymentTokenid'] == $rowAcledaStatus['aclenda_payment_token_id'] && $result['result']['xTran']['invoiceid'] == $rowOrder['code'] && $result['result']['xTran']['purchaseAmount'] == $amount){
                        $checkPayment = true;
                    }
                }
    ...
    if($checkPayment == true){
        ...
        if(mysql_num_rows($sqlTicket)){
            ...
            while($rowTicket = mysql_fetch_array($sqlTicket)){
                ...
                $tTicketId = mysql_insert_id();
                if(empty($tTicketId)){
                    $sysCodeEsc = mysql_real_escape_string($rowTicket['sys_code'], $cn);
                    $sqlFindTicket = mysql_query("SELECT id FROM t_tickets WHERE sys_code = '".$sysCodeEsc."' LIMIT 1", $cn);
                    $rowFindTicket = mysql_fetch_array($sqlFindTicket);
                    if(!empty($rowFindTicket['id'])){
                        $tTicketId = (int)$rowFindTicket['id'];
                    }
                }
                // Move Ticket Detail Tmp to Ticket Detail
                if(!empty($tTicketId)){
                    mysql_query("INSERT INTO t_ticket_details (`sys_code`, `t_ticket_id`, `seat_number`, `label_number`, `gender`, `name`, `telephone`, `unit_price`, `vat_price`, `discount`, `total_amount`, `nationally`) SELECT `sys_code`, ".$tTicketId.", `seat_number`, `label_number`, `gender`, `name`, `telephone`, `unit_price`, `vat_price`, `discount`, `total_amount`, `nationally` FROM t_ticket_detail_api_tmps WHERE t_ticket_api_tmp_id = ".$rowTicket['id'].";");
                }
                mysql_query("UPDATE t_ticket_api_tmps SET status = -3 WHERE id = ".$rowTicket['id'].";");
                // Update Seat Status
                if(!empty($tTicketId)){
                    mysql_query("UPDATE t_seat_controls SET status = 2 WHERE t_ticket_id = ".$tTicketId.";");
                }
            }
        }
    ...
    if(!empty($bankApiResponse)){
        $bankApiResponseEsc = mysql_real_escape_string($bankApiResponse, $cn);
        mysql_query("UPDATE online_orders SET bank_api_response = '".$bankApiResponseEsc."' WHERE id = ".$rowOrder['id'].";");
    }
    // Release Order
    if($release == true){
